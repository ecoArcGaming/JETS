...
class EmpiricalDatasetIMTS(Dataset):
    ...
    def __init__(
        ...
        # Setup cache path
        cache_filename = "cached_imts_dataset.pkl" if is_pretrain else "finetune_cached_imts_dataset.pkl"
        ...
        # Prepare data
        data, targets = self._prepare_data(df, binary_df, min_obs_per_user, min_timespan_days)
        ...
        split_ids = (train_ids, val_ids, test_ids) if not self.is_pretrain else (train_ids, val_ids)
    ...
    def _filter_users_by_binary_df(self, df, binary_df):
        ...
        print(f"Filtered users for finetuning: {final_users} kept, {removed_users} removed")
    ...
    def _prepare_data(self, df, binary_df, min_obs_per_user, min_timespan_days):
        ...
        if self.outlier_method != "none":
            ...
            print(f"After outlier removal: {after_outlier_obs} observations ({initial_obs - after_outlier_obs} removed)")
        ...
        print(f"Users remaining after min_obs filter ({min_obs_per_user}): {users_after_obs_filter} ({initial_users - users_after_obs_filter} removed)")

        print(f"After all cleaning: {len(data_long)} observations for {data_long.ts_id.nunique()} users")
    ...
    def _remove_outliers_optimized(self, data_long):
        ...
        for variable in data_long["variable"].unique():
            ...
            if outlier_count > 0:
                print(f"Variable {variable}: removed {outlier_count} outliers ({outlier_count/len(var_data)*100:.2f}%)")
    ...
    def _create_splits(self, data, train_frac, run):
        ...
        start_train = int(np.linspace(0, len(train_ids_full) - num_train, total_runs)[run_num - 1])
        train_ids = train_ids_full[start_train : start_train + num_train]
        
        num_val = int(np.ceil(train_frac * len(val_ids_full)))
        start_val = int(np.linspace(0, len(val_ids_full) - num_val, total_runs)[run_num - 1])
    ...
    def _preprocess_for_imts(self, data, args, train_ids):
        ...
        # Check if finetuning
        args.finetune = hasattr(args, "load_ckpt_path") and args.load_ckpt_path is not None
        
        if args.finetune:
            # Load normalization stats from pretrained model
            pt_var_path = "normalization_stats.pkl"
            variables, normalization_stats, max_day = pickle.load(open(pt_var_path, "rb"))
            ...
            save_path = getattr(args, "normalization_save_path", "normalization_stats.pkl")
    ...
    def _apply_normalization_vectorized(self, data, normalization_stats):
        ...
        for variable in data.variable.unique():
            ...
            if variable in normalization_stats:
                ...
                print(f"Warning: Variable {variable} not found in normalization stats, using original value")
