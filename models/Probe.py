...
def evaluate_model_multi_target(
    ...
    optimizers = [
        torch.optim.Adam(probe.parameters(), lr=5e-5, weight_decay=1e-2)
        for probe in probes
    ]
...
def create_label_combination_plot(embeddings_2d, labels_array, target_names, figsize):
    ...
    for i in range(num_samples):
        active_targets = [
            target_names[j] for j in range(num_targets) if labels_array[i, j] == 1
        ]
...
if __name__ == "__main__":
    ...
    available_targets = [
        target for target in all_targets if target in binary_df.columns
    ]
    missing_targets = [
        target for target in all_targets if target not in binary_df.columns
    ]

    if missing_targets:
        print(
            f"Warning: The following targets are not available in binary_df: {missing_targets}"
        )

    print(
        f"Evaluating {len(available_targets)} available target conditions simultaneously..."
    )
    print(
        "Available targets:",
        available_targets[:5],
    ...
    try:
        ...
        train_loader, val_loader, dataset = create_multi_target_dataloader(
            args, df, binary_df, available_targets
        )
        ...
        train_percentages, val_percentages = calculate_positive_sample_percentages(
            dataset, available_targets
        )
        print("\nPositive Sample Percentages:")
        for target in available_targets:
            print(
                f"  {target}: Train={train_percentages[target]:.2f}%, Val={val_percentages[target]:.2f}%"
            )
            ...
            results.append(
                {
                    ...
                    "auroc_improvement": (
                        model_auroc - baseline_auroc
                        if (model_auroc is not None and baseline_auroc is not None)
                        else None
                    ),
                    "auprc_improvement": (
                        model_auprc - baseline_auprc
                        if (model_auprc is not None and baseline_auprc is not None)
                        else None
                    ),
        ...
        final_filename = os.path.join(
            output_dir, f"multi_target_evaluation_results_{timestamp}.csv"
        )
        ...
        if len(valid_results) > 0:
            print(
                f"Successfully evaluated: {len(valid_results)}/{len(available_targets)} conditions"
            )
            print(
                f"Average baseline AUROC: {valid_results['baseline_auroc'].mean():.4f}"
            )
            print(f"Average model AUROC: {valid_results['model_auroc'].mean():.4f}")
            print(
                f"Average AUROC improvement: {valid_results['auroc_improvement'].mean():.4f}"
            )
            print(
                f"Average baseline AUPRC: {valid_results['baseline_auprc'].mean():.4f}"
            )
            print(f"Average model AUPRC: {valid_results['model_auprc'].mean():.4f}")
            print(
                f"Average AUPRC improvement: {valid_results['auprc_improvement'].mean():.4f}"
            )

            # Add positive sample percentage statistics
            print(f"\nPositive Sample Statistics:")
            print(
                f"Average train positive %: {valid_results['train_pos_percent'].mean():.2f}%"
            )
            print(
                f"Average val positive %: {valid_results['val_pos_percent'].mean():.2f}%"
            )

            print(f"\nTop 5 conditions with largest AUROC improvements:")
            top_auroc = valid_results.nlargest(5, "auroc_improvement")[
                [
                    "target_condition",
                    "auroc_improvement",
                    "train_pos_percent",
            ...
            for _, row in top_auroc.iterrows():
                print(
                    f"  {row['target_condition']}: +{row['auroc_improvement']:.4f} (Train: {row['train_pos_percent']:.1f}%, Val: {row['val_pos_percent']:.1f}%)"
                )

            print(f"\nTop 5 conditions with largest AUPRC improvements:")
            top_auprc = valid_results.nlargest(5, "auprc_improvement")[
                [
                    "target_condition",
                    "auprc_improvement",
                    "train_pos_percent",
            ...
            for _, row in top_auprc.iterrows():
                print(
                    f"  {row['target_condition']}: +{row['auprc_improvement']:.4f} (Train: {row['train_pos_percent']:.1f}%, Val: {row['val_pos_percent']:.1f}%)"
                )
        else:
