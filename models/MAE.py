...
class MaskedAutoencoder(nn.Module):
    ...
    def forward(self, triplets: torch.Tensor, padding_mask: torch.Tensor, return_representations: bool = False) -> Dict:
        ...
        context_emb = torch.gather(embeddings, 1, context_indices.unsqueeze(-1).expand(-1, -1, embeddings.shape[-1]))
        context_repr = self.encoder(context_emb)
        # Prepare decoder input: context + mask tokens at masked positions
        target_pos_emb = torch.gather(embeddings, 1, target_indices.unsqueeze(-1).expand(-1, -1, embeddings.shape[-1]))
        decoder_input = torch.cat([context_repr, self.mask_token + target_pos_emb], dim=1)
